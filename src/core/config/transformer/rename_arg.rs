...
/// names.
pub struct RenameArgs(IndexMap<String, String>);
...
impl Transform for RenameArgs {
    ...
    fn transform(&self, config: Self::Value) -> Valid<Self::Value, Self::Error> {
        ...
        Valid::from_iter(self.0.iter(), |(existing_name, suggested_name)| {
            if !config.types.values().any(|v| v.fields.values().any(|v| v.args.contains_key(existing_name))) {
                Valid::fail(format!(
                    "Arg '{}' not found in configuration.",
                    existing_name
                ))
            } else {
